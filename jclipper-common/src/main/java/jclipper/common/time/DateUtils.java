...
public final class DateUtils implements Consts {
     ...
     *
     * @param date     日期
     * @param dateType 日期格式,参见{@link Calendar }的类型常量
     * @return 数值
     */
     ...
     *
     * @param date     日期字符串
     * @param dateType 类型,参见{@link Calendar }的类型常量
     * @param amount   数值
     * @return 修改后的日期字符串
     */
        ...
        for (DateStyle style : DateStyle.values()) {
            ...
            if (date != null) {
                try {
                    ...
                    Date dateTmp = getDateFormat(style.value()).parse(date, pos);
                    if (dateTmp != null && pos.getIndex() == date.length()) {
                        return style;
                    }
    ...
    public static Date parseToDate(String date, DateStyle dateStyle) {
        Date myDate = null;
        if (dateStyle != null) {
            myDate = parseToDate(date, dateStyle.value());
        }
        return myDate;
    }
    ...
    public static String parseToString(Date date) {
        return parseToString(date, DEFAULT_DATE_FORMAT);
    }
    ...
    public static String nowToString() {
        return parseToString(Calendar.getInstance().getTime(), DEFAULT_DATE_FORMAT);
    }
    ...
    public static String parseToString(Date date, DateStyle dateStyle) {
        String dateString = null;
        if (dateStyle != null) {
            dateString = parseToString(date, dateStyle.value());
        }
        return dateString;
    }
    ...
    public static String parseToString(String date, DateStyle oldDteStyle, String newPattern) {
        String dateString = null;
        if (oldDteStyle != null) {
            dateString = parseToString(date, oldDteStyle.value(), newPattern);
        }
        return dateString;
    }
    ...
    public static String parseToString(String date, String oldPattern, DateStyle newDateStyle) {
        ...
        if (newDateStyle != null) {
            dateString = parseToString(date, oldPattern, newDateStyle.value());
        }
        return dateString;
    }
    ...
    public static String parseToString(String date, DateStyle oldDteStyle, DateStyle newDateStyle) {
        ...
        if (oldDteStyle != null && newDateStyle != null) {
            dateString = parseToString(date, oldDteStyle.value(), newDateStyle.value());
        }
        return dateString;
    }
     ...
     *
     * @param date         日期字符串
     * @param millisAmount 增加数量。可为负数
     * @return 增加毫秒钟后的日期字符串
     */
    public static String addMillis(String date, int millisAmount) {
        return addInteger(date, Calendar.SECOND, millisAmount);
    }
     ...
     * @return 增加毫秒钟后的日期
     */
    public static Date addMillis(Date date, int millisAmount) {
        return addInteger(date, Calendar.MILLISECOND, millisAmount);
    }
     ...
     *
     * @param date       日期
     * @param dayOfMonth 返回日期在当月的第几天，若此值大于当月的最大天数，这返回当月的最后时刻
     * @return
     */
    public static Date getOtherDayOfMonth(Date date, int dayOfMonth) {
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        int day = Math.min(dayOfMonth, c.getActualMaximum(Calendar.DAY_OF_MONTH));
        c.set(Calendar.DAY_OF_MONTH, day);
        Date result = c.getTime();
        if (day < dayOfMonth) {
            result = getEndOfDay(result);
        }
        return result;
    }
    ...
    public static Date getOtherDayOfWeek(Date date, int dayOfWeek) {
        ...
        return addDay(date, dayOfWeek - day);
    }
    ...
    public static int getIntervalDays(Date date, Date otherDate) {
        int num = -1;
        Date dateTmp = parseToDate(DateUtils.getDate(date), DateStyle.YYYY_MM_DD);
        Date otherDateTmp = parseToDate(DateUtils.getDate(otherDate), DateStyle.YYYY_MM_DD);
